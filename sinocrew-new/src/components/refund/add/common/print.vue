  ...
  methods:{
    ...
    getPrintData(){
      ...
      if(Object.keys(this.printData).length>0){
          ...
          if(routerpath.indexOf("/refundRecordOffice/info/")>-1){
            this.list2.push(["","","应补金额(大写)",_g.Arabia_To_SimplifiedChinese(this.amount[this.amount.length-1]-ship_print[ship_print.length-1])])
          ...
          this.list3.forEach(ele=>{
            if(this.printData.checkin.roleId==1){
              if(ele.key){
                ele.value="系统管理员"
              }
            }else{
              // 非系统管理员
              if(ele.key){
                ele.value=this.printData.checkin?(this.printData.checkin[ele.key]?this.printData.checkin[ele.key]:"无"):"无"
              }
            }
          })
