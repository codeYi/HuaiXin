    ...
    <table class="noprint">
        ...
        <td v-for="item in tableModel" :class="item.isBig?'is-big':''">
            ...
            <span :class="item.grow||'grow_2_5'">
              <el-autocomplete
                ...
                :disabled="canEdit"
              ></el-autocomplete>
              <el-select clearable filterable v-else-if="item.type=='select'" v-model="item.value" placeholder="" @change="changeAddParams(item)" :disabled="canEdit">
          ...
          <span v-else>
            <div class="canEdit-box">
              ...
              <span v-else-if="one.type=='select'">
                <el-select clearable filterable v-model="one.value" placeholder="" filterable :title="one.value" @change="changeAddParams(one,index1,'fee')" :disabled="canEdit">
      ...
      <tr :class="footClassName?footClassName+' candelete':''" v-if="footTableModel.length>0" v-for="(item,index) in footTableModel" :title="canDelete?'双击可以删除这一行':''" @dblclick="deleteThis(index)">
        <td v-for="(one,index1) in item" :class="one.class">
          ...
          <el-select clearable filterable v-else-if="one.type=='select'" filterable v-model="one.value" @change="changeThis(one,item);changeAddParams(one,index,'data')" :disabled="canEdit">
              <el-option v-for="(two,index2) in one.array" :key="index2" :label="two.title||two" :value="two.id>-100?two.id:two"></el-option>
          </el-select>
        ...
        <div v-if="footMoreData.length>0">
          <tr class="for-refund" v-for="(item,index) in footMoreData" v-show="showArray.length==0?true:!showArray.includes(index)">
            <td v-for="one in item" :class="one.class" :colspan="one.colspan">
              ...
              <el-input v-if="one.type=='input'" v-model="one.value" :placeholder="one.readonly?'':'请填写'" :readonly="staffEditMoney||one.readonly" @change="changeMoney"></el-input>
      ...
      <tr class="foot-info sumbit-box" v-if="showBtnBox">
        <td>
          <span v-if="btns==1||showSubmitBtn" :class="canSaveSuccess?'my-btn submit-btn':'my-btn submit-btn bac-grey'" @click="saveThis(0)">提交</span>
          <span v-if="shiManyBtn" :class="canSaveSuccess?'my-btn submit-btn':'my-btn submit-btn bac-grey'" @click="addMany">批量</span>
          <span v-if="btns==2&&showPassBtns" :class="canSaveSuccess?'my-btn no-pass-btn':'my-btn no-pass-btn bac-grey'" @click="saveThis(2)">不通过</span>
          <span v-if="btns==2&&showPassBtns" :class="canSaveSuccess?'my-btn submit-btn':'my-btn submit-btn bac-grey'" @click="saveThis(1)">通过</span>
...
export default {
  ...
  data() {
    return {
      ...
      reasons2:["访船","培训","招生","拜访客户","拜访船员","参会","其他"],
      supplier:[],
      shiperList:[],
  ...
  methods:{
    ...
    countAll(value){
      ...
      this.footTableModel.forEach((ele,index)=>{
        if(routerpath!="/refundAddOffice"&&!(routerpath.indexOf("/refundRecordOffice/info/")>-1)){
          ...
          ele.forEach((ele2,index2)=>{
            ...
            if(routerpath.indexOf("/refundSignBoater/info/")>-1||routerpath=="/refundAddBoaters"||routerpath.indexOf("/refundRecordBoaters/info/")>-1){
              if(index2>2&&index2<ele.length-3&&index2!=10){
                var num=(Number(this.amount[index2])+Number(ele2.value)).toFixed(2)
          ...
          ele[ele.length-1].value=ele[ele.length-1].value.toFixed(2)
          this.amount[this.amountData.length-1]=(this.amount[this.amountData.length-1]+Number(ele[ele.length-1].value)).toFixed(2)
        }else{
          this.amount[this.amountData.length-1]=(this.amount[this.amountData.length-1]+Number(ele[ele.length-1].value)).toFixed(2)
        }
      })
    ...
    countAllShip(item,one){
      ...
      if(routerpath!=="/refundAddOffice"){
        if(!(routerpath.indexOf("/refundSignOffice/info/")>-1)){
          ...
          setTimeout(()=>{
            this.$set(one,"value",one.value||0)
            this.shipProject[2][3].value=(this.amount[this.amount.length-1]-one.value)<0?0:(this.amount[this.amount.length-1]-one.value)
            this.submitParams.borrow=one.value
            this.submitParams.really=this.shipProject[2][3].value
            this.shipProject[1][1].value=_g.Arabia_To_SimplifiedChinese(this.shipProject[2][3].value)
          },100)
        }
      }
    ...
    changeData(){
      ...
      this.apiPost("expenses/getCity").then(res=>{

        _g.getMyItem(this.tableModelData,"label","报销地点").value=res[0]
        this.AddParams.address=res[0]
      })
      
      // 获取查查看数据
      if(routerpath=="/refundAddBoaters"){
        this.shiManyBtn=true
        this.AddParams={
          mariner_id:"",
          id:"",
          date:"",
        ...
        this.apiPost(this.urls.sign,{id}).then(res=>{
          ...
          if(routerpath.indexOf("/refundSignBoater/info")>-1){
            this.officeMoney=(localStorage.getItem("isPassed")!=="待签批")||(res.editor==1?false:true)
            this.canEdit=(localStorage.getItem("isPassed")!=="待签批")||(res.editor==0?true:false)
          }
        ...
        this.apiPost(this.urls.sign,{id}).then(res=>{
          ...
          this.apiPost(this.urls.sign,{id}).then(res=>{
            ...
            var array5=["project","explain_cost","number","total"]
            var result=0
      ...
      if(index==0){
        ...
        if(this.checkInfo(this.tableModel)&&this.checkFootInfo(this.footTableModel)){
          this.apiPost(this.urls.addUrl,this.AddParams).then(res=>{
            ...
            if(!res.error){
              // 提交成功
              this.canSaveSuccess=false
              setTimeout(()=>{
                this.canSaveSuccess=true
              },3000)
        ...
        if(routerpath.indexOf("/refundSignBoater/info/")>-1){
          ...
          for(var item of this.footTableModel){
            ...
            for(var item1 of item){
              if(item1.key)obj[item1.key]=item1.value
              if(item1.key=="number"&&item1.value)this.AddParams.number.push(item1.value)
            }
            this.AddParams.data.push(obj)
          }
          ...
          this.apiPost("Expenses/marinerAdd",this.AddParams).then(res=>{
            if(res.error){
              _g.toMessage(this,"error","船员消息更新失败")
            }
          })
    ...
    checkInfo(array){
      ...
      for(let i=0 ; i<array.length;i++){
        if(array[i].value||(router=="/refundAddStaffs"&&array[i].label=="船名")||(router=="/refundAddStaffs"&&array[i].label=="客户")||(router=="/refundAddOffice"&&array[i].label=="客户")){
          continue 
        }else{
    ...
    getSelectdata(){
      ...
      var a= setInterval(()=>{
        this.customs=this.$store.state.tableListData.owners
        // console.log(this.$store.state.tableListData.owners)
    ...
    getIsPassed(){
      ...
      if(router.indexOf("/refundRecordBoaters/info")>-1){
        ...
        this.staffEditMoney=status=="待签批"?false:true
        this.officeMoney=status=="待签批"?false:true
  ...
  components: {Print},
  created() {
    this.showMore=localStorage.getItem("mariner")==0
