  ...
  <div class="chart">
    <div v-for="(item,index0) in chartList" class="first-box box-size border-common" v-show="item.show">
      <div v-show="item.showTitle" class="title box-size">
        ...
        <div class="is-more" v-if="(item.options&&item.options.length)&&!item.showFirst">
        ...
        <div class="is-more" v-if="item.options2&&item.options2.length&&!item.showSecond" style="margin-left:20px;">
          <span class="statistics-type">统计方式</span>
...
export default {
  ...
  methods:{
    ...
    detail_6(params){
      var goal=this.chartList[6]
      if(params.seriesName!=="报销总额"&&goal.params.type==1){
    ...
    doDetails(index,params){
      ...
      if(index==5){
        ...
        if(goal.params2.type=="mariner"){
          name="船员差旅"
        }else if(goal.params2.type=="user"){
          name="员工差旅"
        ...
        this.apiPost("index/expenseType",goal.params2).then(res=>{
          ...
          if(goal.params2.type=="mariner"){//船员
            ...
            goal.option2.series[0].data.forEach(ele3=>{
              
              if(!ele3.value){
                goal.option2.legend.selected[ele3.name]=false
              }
            })
            ...
            res.forEach(ele0 =>{
              goal.option2.legend.data.push(ele0.department||ele0.shipowner||ele0.address)
              goal.option2.series[0].data.push({name:ele0.department||ele0.shipowner||ele0.address,value:ele0.total})
            })
            res.forEach(ele0 =>{
              var second=ele0.result
              second.forEach((ele1,index1)=>{
                  if(!goal.option2.legend.data.includes(ele1.project))goal.option2.legend.data.push(ele1.project)
                  goal.option2.series[1].data.push({
                    name:ele1.project,
                    value:ele1.money
                  })
            ...
            goal.option2.series[0].data.forEach(ele3=>{
              if(!ele3.value){
                ...
                // goal.option2.legend.selected[ele3.name]=true
              }
            })
            goal.option2.series[1].data.forEach(ele3=>{
              if(!ele3.value){
                goal.option2.legend.selected[ele3.name]=false
              }else{
            ...
            // console.log(goal.option2.series[1].data)

            // goal.option2.legend.data=goal.option2.legend.data.concat(array1)
          }else if(goal.params2.type=="user"){
            var array1=["市内交通费","住宿费","交通费","差旅补贴","招待餐费","其他交际费","办公杂费","通讯费","邮递费","同行人","其他"]
            var array2=["city","hotel","traffic","travel","meal","exchange","office","communication","post","partner","rests"]
            res.forEach(ele=>{
              goal.option2.legend.data.push(ele.department||ele.shipowner||ele.address||ele.vessel)
              goal.option2.series[0].data.push({name:ele.department||ele.shipowner||ele.address||ele.vessel,value:ele.total})
      ...
      array.forEach((ele,index_time)=>{
        if(ele.url&&ele.show){
          this.apiPost(ele.url,ele.params).then(res=>{
            ...
            ele.option.series[0].data=[]
            if(ele.option.series[1])ele.option.series[1].data=[]
            ...
            }else if(ele.id=="chart11"){
              ele.option.legend.data=[]
              // ele.option.series[0].name="test"
            }
            ...
            if(res)res.forEach(ele1=>{
              ele.option.xAxis[0].data.push(ele1.month||ele1.area||ele1.title||ele1.address||ele1.department||ele1.name||"暂无")
              if(ele.id!="chart8"&&ele.id!="chart11"){
                ...
                ele.option.series[0].data.push(ele1.total||ele1.number)
                ele.option.legend.data.push(ele1.department||ele1.name)
              }
            })
          }).then(()=>{
            ele.chart=ele.chart
...
.chart > div:nth-child(6),.chart > div:nth-child(7),.chart > div:nth-child(8),.chart > div:nth-child(9),.chart > div:nth-child(10),.chart > div:nth-child(11){
  width: 100%;
  margin-left: 0;
  margin-right: 0;
}
