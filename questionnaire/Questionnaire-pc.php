...
class Questionnaire extends PcBase
{
    ...
    public function addVote(Request $request)
    {
        ...
        Db::startTrans();
        try {
            $insertId = Db::name('vote')->insertGetId($insert);
            foreach ($option as $k => $v) {
                $option[$k]['pid'] = $insertId;
            }
            Db::name('vote_options')->insertAll($option);
            Db::commit();
            return ok_data();
        } catch (\Exception $e) {
            Db::rollback();
    ...
    {
        $id = input('id');
        $info = Db::name('vote_options')->where(['pid' => $id])->select();
        foreach ($info as $k => $v) {
